...
ind.setEmVias(True)
ind.setupGeometry(r_outer, w, s, nturns, sig_lay, underpass_lay, underpass_lay, ind_geom)
ind_name = "inductorSymCT_" + ind_geom + "_r_outer" + str(r_outer) + "_w" +  str(w) + "_n" + str(nturns) + "_s" + str(s) + "_emvias"
ind.genGeometry()
ind.genGDSII(ind_name + '.gds', structName = ind_name)

# Generate square inductor
ind.setEmVias(False)
ind_geom = "rect"
ind.setupGeometry(r_outer, w, s, nturns, sig_lay, underpass_lay, underpass_lay, ind_geom)
ind_name = "inductorSymCT_" + ind_geom + "_r_outer" + str(r_outer) + "_w" +  str(w) + "_n" + str(nturns) + "_s" + str(s)
...
# Generate layout with substrate ring
ind_geom = "octagon"
ind.setupGeometry(r_outer, w, s, nturns, sig_lay, underpass_lay, underpass_lay, ind_geom, subRingSpace = subRingSpace, subRingW = subRingW, diffLayer = diffLayer, implantLayer = implantLayer)
ind_name = "inductorSymCT_" + ind_geom + "_r_outer" + str(r_outer) + "_w" +  str(w) + "_n" + str(nturns) + "_s" + str(s) + "_subring"
ind.genGeometry()
ind.genGDSII(ind_name + '.gds', structName = ind_name)
